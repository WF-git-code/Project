...
int main() {
    ...
    FixedThreadPool pool(QUEUE_SIZE, WAIT_TIME, THREAD_NUM);
    std::cout << "线程池配置：" << std::endl;
    std::cout << "  队列大小：" << QUEUE_SIZE << std::endl;
    std::cout << "  工作线程：" << THREAD_NUM << std::endl;
    ...
    std::cout << "File Task " << failed_tasks << std::endl;
    std::cout << "result:" << total_result << std::endl;
    ...
    std::atomic_int a;
    a.store(a, std::memory_order_relaxed);
